...
const isTemplateLiteralArgs = (args) => {
...
const initWithNodeName = (nodeName, namespaceURI) => (...args) => {
  ...
  const attrs = isTemplateLiteralArgs(args) ? parseAbbr(parseTemplate(args[0], ...args.slice(1))) : args[0];
  return Object.assign(initWithNodeNameAndAttrs(nodeName, namespaceURI, attrs), _rawComponent(nodeName, namespaceURI, attrs))
};

const init = (nodeName: string, namespaceURI?: string): any => Object.assign(initWithNodeName(nodeName, namespaceURI), _rawComponent(nodeName, namespaceURI));
